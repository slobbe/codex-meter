...
export async function fetchCodexUsageSnapshot() {
    ...
    return {
        ...
        fiveHour: normalizeWindow(usage?.rate_limit?.primary_window),
        weekly: normalizeWindow(usage?.rate_limit?.secondary_window),
        subscription,
    };
}
...
function normalizeWindow(window) {
    ...
    return {
        ...
        limitWindowSeconds: Number.isFinite(window.limit_window_seconds) ? window.limit_window_seconds : null,
        resetAfterSeconds: Number.isFinite(window.reset_after_seconds) ? window.reset_after_seconds : null,
        resetAt: Number.isFinite(window.reset_at) ? window.reset_at : null,
    };
}
