...
import {
    ...
    readCachedUsageSnapshot,
    readUsageHistory,
} from "./src/old-codex.js";
...
class CodexMeterIndicator extends PanelMenu.Button {
    ...
    async _loadCachedSnapshot() {
        const snapshot = await readCachedUsageSnapshot();

        if (snapshot) this._snapshot = snapshot;
    }

    async _loadHistory() {
        this._historyRows = await readUsageHistory();
    }

    _getRefreshIntervalSeconds() {
        const intervalMinutes = this._settings.get_uint(
    ...
    _syncLabel() {
        const showFiveHour = this._settings.get_boolean(
            SETTINGS_SHOW_FIVE_HOUR,
        );
        const showWeekly = this._settings.get_boolean(SETTINGS_SHOW_WEEKLY);
        const displayMode = this._settings.get_string(
        ...
        const showUnifiedBar =
            displayMode === "unified" && this._snapshot && hasTopBarUsage;
        const showSplitBars =
            displayMode === "bars" && this._snapshot && hasTopBarUsage;
        ...
        if (this._snapshot) {
            if (showUnifiedBar) {
                if (showFiveHour && showWeekly) {
                    this._panelFiveHourBar.percentValue =
                        calculateUnifiedPercent(
                            this._snapshot.fiveHour?.usedPercent,
                            this._snapshot.weekly?.usedPercent,
                        );
                } else if (showFiveHour) {
                    this._panelFiveHourBar.percentValue = normalizePercent(
                        this._snapshot.fiveHour?.usedPercent,
                    );
                } else {
                    this._panelFiveHourBar.percentValue = normalizePercent(
                ...
                this._panelWeeklyBar.percentValue = normalizePercent(
                    this._snapshot.weekly?.usedPercent,
                );
            }
        ...
        if (showSplitBars || showUnifiedBar) return;
    ...
    _syncMenu() {
        ...
        this._headerItem.datetimeLabel.text = formatUpdatedAt(
            this._snapshot.fetchedAt,
        );
        this._setUsageItem(
            this._fiveHourItem,
            "Session (5h)",
        ...
        this._footerItem.planLabel.text = formatPlan(
            this._snapshot.subscription?.planType ?? this._snapshot.planType,
        );
    }
...
function predictLimitHit(
    window,
    snapshot,
    historyRows,
    key,
    now = Date.now(),
    requirements = SESSION_PREDICTION_REQUIREMENTS,
) {
    if (!window) return createPrediction("unavailable");

    const latestPercent = clampPredictionPercent(window.usedPercent);

    if (!Number.isFinite(latestPercent)) return createPrediction("unavailable");

    if (latestPercent >= 100)
        return createPrediction("already-limited", now, 0);

    const resetAt = Number.isFinite(window.resetAt)
        ? window.resetAt * 1000
        : null;
    ...
    if (Number.isFinite(currentTime)) {
        ...
        if (
            !lastSample ||
            lastSample.time !== currentTime ||
            lastSample.percent !== latestPercent
        )
    ...
    if (
        elapsedMs < requirements.minTimespanMs ||
        percentGrowth < requirements.minGrowthPercent
    )
        return createPrediction("unavailable");
    ...
    const secondsUntilLimit = Math.max(
        0,
        Math.round((predictedAt - now) / 1000),
    );
...
function calculateTrendSlope(samples) {
    ...
    const meanTime =
        samples.reduce((sum, sample) => sum + (sample.time - originTime), 0) /
        samples.length;
    const meanPercent =
        samples.reduce((sum, sample) => sum + sample.percent, 0) /
        samples.length;
    let covariance = 0;
    let variance = 0;
...
function getPredictionStyleClass(prediction) {
    switch (prediction?.status) {
        case "already-limited":
            return "danger";
        case "before-reset":
            return prediction.secondsUntilLimit < 30 * 60
                ? "danger"
                : "warning";
        case "safe":
            return "safe";
