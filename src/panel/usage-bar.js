...
import { calculateBarFillWidth, calculateBarMarkerPosition, getUsageBarColorStyleClass, } from "./view-model.js";
...
export class UsageBar {
    ...
    sync() {
        ...
        this._barFill.width = calculateBarFillWidth(width, this._state.displayPercentValue);
        if (!this._barMarker)
            return;
        this._barMarker.width = BASELINE_MARKER_WIDTH;
        this._barMarker.height = this.actor.height;
        this._barMarker.visible = Number.isFinite(this._state.baselinePercentValue);
        this._barMarker.x = calculateBarMarkerPosition(width, this._barMarker.width, this._state.displayBaselinePercentValue);
        this._barMarker.y = 1;
    }
    _syncColor() {
        ...
        this._barFill.add_style_class_name(getUsageBarColorStyleClass(this._state.percentValue));
    }
}
