...
function parseApiResponse(text, url, config, allowArrayResponse = false) {
    ...
    try {
        ...
        throw new RefreshFailureError("malformed-response", config.messages.malformedResponse, `Failed to parse JSON response from ${url}: ${message}`);
    }
    const isObject = typeof parsed === "object" && parsed !== null;
    const isArray = Array.isArray(parsed);
    if (!isObject || (isArray && !allowArrayResponse)) {
        throw new RefreshFailureError("malformed-response", config.messages.unexpectedResponseFormat, `Expected JSON ${allowArrayResponse ? "object or array" : "object"} response from ${url}`);
    }
    return parsed;
}
export async function fetchJson(url, config, options = {}) {
    ...
    let bytes;
    try {
        bytes = await session.send_and_read_async(message, GLib.PRIORITY_DEFAULT, options.cancellable ?? null);
    }
    catch (error) {
        ...
        throw new RefreshFailureError("network", config.messages.networkUnavailable, `Request to ${url} failed before receiving a response: ${message}`);
    }
    if (bytes.get_size() > MAX_RESPONSE_BYTES) {
        throw new RefreshFailureError("malformed-response", config.messages.responseTooLarge, `Response from ${url} exceeded ${MAX_RESPONSE_BYTES} bytes`);
    }
    ...
    if (message.statusCode < 200 || message.statusCode >= 300) {
        ...
        if (message.statusCode === 401 || message.statusCode === 403) {
            throw new RefreshFailureError("unauthorized", config.messages.unauthorized, technicalMessage);
        }
        throw new RefreshFailureError("network", config.messages.refreshFailed, technicalMessage);
    }
    if (!data) {
        throw new RefreshFailureError("malformed-response", config.messages.emptyResponse, `Received empty response body from ${url}`);
    }
