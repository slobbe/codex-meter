import { getQuota, getSecondaryQuota } from "../domain/usage.js";
export function createPanelBarViewModel(settings, snapshot, errorMessage) {
    ...
    if (snapshot) {
        ...
        viewModel.primaryDisplayPercent = convertPercentForDisplay(viewModel.primaryPercent, percentDisplayMode);
        viewModel.secondaryDisplayPercent = convertPercentForDisplay(viewModel.secondaryPercent, percentDisplayMode);
    }
...
export function createMenuViewModel(settings, snapshot, prediction, history = [], errorMessage = null, cachedFailureMessage = null) {
    const percentDisplayMode = settings.percentDisplayMode;
    if (errorMessage) {
        return {
            updatedAt: "--",
    ...
    return {
        ...
        primary: createUsageItemViewModel({
            ...
            prediction: formatLimitPrediction(primaryQuota ? prediction?.quotas?.[primaryQuota.id] : prediction?.primary, primaryQuota, "primary"),
            ...
            percentDisplayMode,
            predictionStyle: getPredictionStyleClass(primaryQuota ? prediction?.quotas?.[primaryQuota.id] : prediction?.primary),
        }),
        secondary: createUsageItemViewModel({
            ...
            prediction: formatLimitPrediction(secondaryQuota ? prediction?.quotas?.[secondaryQuota.id] : prediction?.secondary, secondaryQuota, "secondary"),
            ...
            percentDisplayMode,
            predictionStyle: getPredictionStyleClass(secondaryQuota ? prediction?.quotas?.[secondaryQuota.id] : prediction?.secondary),
        }),
        plan: formatFooter(snapshot),
        trend: createUsageTrendViewModel(snapshot, history),
    };
}
export function createUsageItemViewModel({ visible = true, title, value, prediction = "", reset = "resets in --", percentValue = null, baselinePercentValue = null, percentDisplayMode = "used", predictionStyle = "muted", }) {
...
export function createUsageTrendViewModel(snapshot, history = [], nowSeconds = Date.now() / 1000) {
    ...
    if (samples.length < 2)
        return createHiddenUsageTrendViewModel();
...
function addTrendDeltaToBuckets({ bucketValues, delta, fromTimestamp, toTimestamp, minTimestamp, bucketSize, }) {
...
function getTrendBucketIndex(timestamp, minTimestamp, bucketSize) {
    return Math.min(TREND_BUCKET_COUNT - 1, Math.max(0, Math.floor((timestamp - minTimestamp) / bucketSize)));
}
...
export function calculateBarMarkerPosition(trackWidth, markerWidth, percentValue) {
    if (trackWidth <= 0 || !Number.isFinite(percentValue))
        return 0;
...
export function calculateBaselinePercent(quota) {
    ...
    if (!Number.isFinite(limitWindowSeconds) ||
...
export function getUsageBarColorStyleClass(percentValue) {
    ...
    if (percent >= 100)
        return "cx-color-danger";
    if (percent > 75)
        return "cx-color-warning";
...
export function formatReset(quota, windowType) {
    if (!quota)
        return "resets in --";
    if (quota.resetDescription)
        return quota.resetDescription;
...
export function formatLimitPrediction(prediction, quota, windowType) {
    if (prediction?.trend === "limit reached")
        return "Limit reached";
    if (prediction?.trend === "safe")
    ...
    if (Number.isFinite(quota?.resetAt)) {
        const secondsBeforeReset = Math.max(0, Math.round(quota?.resetAt - prediction.estimatedLimitAt));
...
function formatCredits(credits) {
    ...
    if (!balance || isZeroCreditBalance(balance))
        return "";
